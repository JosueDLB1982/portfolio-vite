...
</footer>`,p=document.createElement("a"),y=()=>{document.querySelector("#download-btn").addEventListener("click",()=>{setTimeout(()=>{p.href="./assets/documents/cv-enero-2024.pdf",p.download="Josue Lopez Frontend DEV",p.click()},3200)}),document.querySelectorAll(".button").forEach(e=>{let r=3e3,n=e.querySelector("svg"),a=new Proxy({y:null,smoothing:null},{set(s,t,l){return s[t]=l,s.y!==null&&s.smoothing!==null&&(n.innerHTML=o(s.y,s.smoothing,null)),!0},get(s,t){return s[t]}});e.style.setProperty("--duration",r),a.y=20,a.smoothing=0,e.addEventListener("click",s=>{e.classList.contains("loading")||(e.classList.add("loading"),gsap.to(a,{smoothing:.3,duration:r*.065/1e3}),gsap.to(a,{y:12,duration:r*.265/1e3,delay:r*.065/1e3,ease:Elastic.easeOut.config(1.12,.4)}),setTimeout(()=>{n.innerHTML=o(0,0,[[3,14],[8,19],[21,6]])},r/2))})});function i(e,r,n,a){let s=(c,u,b,w)=>{let d=u||c,m=b||c,f={length:Math.sqrt(Math.pow(m[0]-d[0],2)+Math.pow(m[1]-d[1],2)),angle:Math.atan2(m[1]-d[1],m[0]-d[0])},h=f.angle+(w?Math.PI:0),v=f.length*a;return[c[0]+Math.cos(h)*v,c[1]+Math.sin(h)*v]},t=s(n[r-1],n[r-2],e,!1),l=s(e,n[r-1],n[r+1],!0);return`C ${t[0]},${t[1]} ${l[0]},${l[1]} ${e[0]},${e[1]}`}function o(e,r,n){return`<path d="${(n||[[4,12],[12,e],[20,12]]).reduce((t,l,c,u)=>c===0?`M ${l[0]},${l[1]}`:`${t} ${i(l,c,u,r)}`,"")}" />`}},j=g=>{(()=>{const i=document.createElement("div");i.innerHTML=_,document.querySelector(g).append(i)})(),y()};j("#app");
