...
</footer>`,y=()=>{f("dark")},w=()=>{f("light")},f=d=>{const i=document.querySelector(".light-mode-btn"),c=document.querySelector(".dark-mode-btn");i.addEventListener("click",()=>{y()}),c.addEventListener("click",()=>{w()}),document.documentElement.setAttribute("data-theme",d)},p=document.createElement("a"),L=()=>{document.querySelector("#download-btn").addEventListener("click",()=>{setTimeout(()=>{p.href="./assets/documents/cv-enero-2024.pdf",p.download="Josue Lopez Frontend DEV",p.click()},3200)}),document.querySelectorAll(".button").forEach(e=>{let r=3e3,n=e.querySelector("svg"),a=new Proxy({y:null,smoothing:null},{set(s,t,l){return s[t]=l,s.y!==null&&s.smoothing!==null&&(n.innerHTML=c(s.y,s.smoothing,null)),!0},get(s,t){return s[t]}});e.style.setProperty("--duration",r),a.y=20,a.smoothing=0,e.addEventListener("click",s=>{e.classList.contains("loading")||(e.classList.add("loading"),gsap.to(a,{smoothing:.3,duration:r*.065/1e3}),gsap.to(a,{y:12,duration:r*.265/1e3,delay:r*.065/1e3,ease:Elastic.easeOut.config(1.12,.4)}),setTimeout(()=>{n.innerHTML=c(0,0,[[3,14],[8,19],[21,6]])},r/2))})});function i(e,r,n,a){let s=(o,u,_,k)=>{let m=u||o,g=_||o,h={length:Math.sqrt(Math.pow(g[0]-m[0],2)+Math.pow(g[1]-m[1],2)),angle:Math.atan2(g[1]-m[1],g[0]-m[0])},v=h.angle+(k?Math.PI:0),b=h.length*a;return[o[0]+Math.cos(v)*b,o[1]+Math.sin(v)*b]},t=s(n[r-1],n[r-2],e,!1),l=s(e,n[r-1],n[r+1],!0);return`C ${t[0]},${t[1]} ${l[0]},${l[1]} ${e[0]},${e[1]}`}function c(e,r,n){return`<path d="${(n||[[4,12],[12,e],[20,12]]).reduce((t,l,o,u)=>o===0?`M ${l[0]},${l[1]}`:`${t} ${i(l,o,u,r)}`,"")}" />`}},M=d=>{(()=>{const i=document.createElement("div");i.innerHTML=j,document.querySelector(d).append(i)})(),L(),f(),y(),w()};M("#app");
