(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))e(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&e(i)}).observe(document,{childList:!0,subtree:!0});function l(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function e(r){if(r.ep)return;r.ep=!0;const n=l(r);fetch(r.href,n)}})();const w=`<header class="header">\r
  ...
  <button aria-label="activar modo claro" class="light-mode-btn">\r
    <img src="assets/images/moon.svg" alt="imagen luna">\r
      ...
      <ul class="mobile-menu">\r
          ...
          <li>\r
            <a href="#projects" id="id" class="menu-item">Proyectos</a>\r
          </li>\r
          <li>\r
...
</footer>`,y=()=>{const t=window.matchMedia("(prefers-color-scheme: dark)"),s=r=>{document.documentElement.setAttribute("data-theme",r)};t.matches&&s("dark"),t.addEventListener("change",r=>{s(r.matches?"dark":"light")});const l=document.querySelector(".light-mode-btn"),e=document.querySelector(".dark-mode-btn");l.addEventListener("click",()=>{s("dark")}),e.addEventListener("click",()=>{s("light")})},p=document.createElement("a"),_=()=>{document.querySelector("#download-btn").addEventListener("click",()=>{setTimeout(()=>{p.href="./assets/documents/cv-enero-2024.pdf",p.download="Josue Lopez Frontend DEV",p.click()},3200)}),document.querySelectorAll(".button").forEach(e=>{let r=3e3,n=e.querySelector("svg"),i=new Proxy({y:null,smoothing:null},{set(a,c,o){return a[c]=o,a.y!==null&&a.smoothing!==null&&(n.innerHTML=l(a.y,a.smoothing,null)),!0},get(a,c){return a[c]}});e.style.setProperty("--duration",r),i.y=20,i.smoothing=0,e.addEventListener("click",a=>{e.classList.contains("loading")||(e.classList.add("loading"),gsap.to(i,{smoothing:.3,duration:r*.065/1e3}),gsap.to(i,{y:12,duration:r*.265/1e3,delay:r*.065/1e3,ease:Elastic.easeOut.config(1.12,.4)}),setTimeout(()=>{n.innerHTML=l(0,0,[[3,14],[8,19],[21,6]])},r/2))})});function s(e,r,n,i){let a=(d,u,b,k)=>{let m=u||d,g=b||d,f={length:Math.sqrt(Math.pow(g[0]-m[0],2)+Math.pow(g[1]-m[1],2)),angle:Math.atan2(g[1]-m[1],g[0]-m[0])},h=f.angle+(k?Math.PI:0),v=f.length*i;return[d[0]+Math.cos(h)*v,d[1]+Math.sin(h)*v]},c=a(n[r-1],n[r-2],e,!1),o=a(e,n[r-1],n[r+1],!0);return`C ${c[0]},${c[1]} ${o[0]},${o[1]} ${e[0]},${e[1]}`}function l(e,r,n){return`<path d="${(n||[[4,12],[12,e],[20,12]]).reduce((c,o,d,u)=>d===0?`M ${o[0]},${o[1]}`:`${c} ${s(o,d,u,r)}`,"")}" />`}},j=()=>{const t=document.querySelectorAll(".menu-item"),s=document.querySelector(".checkbox");t.forEach(l=>{l.addEventListener("click",()=>{s.checked&&(s.checked=!1)})})},L=t=>{(()=>{const s=document.createElement("div");s.innerHTML=w,document.querySelector(t).append(s)})(),y(),_(),j()};L("#app");
